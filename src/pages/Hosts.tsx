...
const Hosts = () => {
  const [hasSeenOnboarding, setHasSeenOnboarding] = useState<boolean>(localStorage.getItem('hostsOnboardingSeen') === 'true');
  useEffect(() => {
  ...
  const handleAddServersToProfile = (servers: ServerInstance[]) => {
    ...
    const newServers = servers.filter(server => !serverInstances.some(existingServer => existingServer.id === server.id));
    ...
    if (selectedProfileId) {
      setProfilesList(prev => prev.map(profile => {
        if (profile.id === selectedProfileId) {
          const updatedInstances = [...profile.instances, ...newServerIds.filter(id => !profile.instances.includes(id))];
          return {
            ...profile,
  ...
  if (!hasSeenOnboarding) {
    return <Welcome onAddHosts={handleOpenAddHostDialog} onSkip={handleCompleteOnboarding} />;
  }
  if (hostsList.length === 0) {
    return <HostsEmptyState onAddHost={handleOpenAddHostDialog} />;
  }
  return <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 min-h-0">
        <CollapsibleHostsLayout hostsList={<div className="p-4 space-y-2">
              {filteredHosts.map(host => <Card key={host.id} className={`cursor-pointer transition-colors hover:bg-muted/50 ${selectedHostId === host.id ? 'border-primary bg-primary/5' : ''}`} onClick={() => setSelectedHostId(host.id)}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5">
                          <div className={`w-2 h-2 rounded-full ${host.connectionStatus === "connected" ? 'bg-green-500' : 'bg-neutral-400'}`} />
            ...
            </div>} hostsListCollapsed={<CollapsedHostsList hosts={filteredHosts} selectedHostId={selectedHostId} onHostSelect={setSelectedHostId} />} hostDetails={selectedHost ? <HostDetailView host={selectedHost} profiles={profilesList} serverInstances={serverInstances} selectedProfileId={selectedProfileId} onCreateConfig={handleCreateConfigDialog} onProfileChange={handleProfileChange} onAddServersToHost={handleAddServersToHost} onDeleteHost={handleDeleteHost} onServerStatusChange={handleServerStatusChange} onSaveProfileChanges={handleSaveProfileChanges} onCreateProfile={handleCreateProfile} onDeleteProfile={handleDeleteProfile} onAddServersToProfile={handleAddServersToProfile} onStartAIChat={handleStartAIChatFromHost} /> : <div className="border border-dashed rounded-md p-8 text-center space-y-3">
              ...
              </div>} chatPanel={<InlineChatPanel onToggleChat={handleToggleChat} />} isChatOpen={isChatOpen} onToggleChat={handleToggleChat} />
      </div>
      
      <UnifiedHostDialog open={unifiedHostDialogOpen} onOpenChange={setUnifiedHostDialogOpen} onAddHosts={handleAddHosts} />
