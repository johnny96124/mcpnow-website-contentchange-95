              ...
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-2">
                    ...
                    {definition?.isOfficial && <Badge variant="outline" className="bg-white/10 text-white border-white/20">
                        Official
                      </Badge>}
                  </div>
                </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-1 flex flex-col">
            ...
            <div className="flex-1 overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-6">
                      ...
                      {definition?.features && <div>
                          <h3 className="text-base font-semibold mb-3 text-gray-800 dark:text-gray-200">
                            Features
                          </h3>
                          <ul className="list-disc list-inside space-y-1.5 text-sm text-gray-600 dark:text-gray-300 pl-1">
                            {definition.features.map((feature, index) => <li key={index}>{feature}</li>)}
                          </ul>
                        </div>}
                      
                      {definition?.categories && <div>
                          <h3 className="text-base font-semibold mb-3 text-gray-800 dark:text-gray-200">
                            Categories
                          </h3>
                          <div className="flex flex-wrap gap-2">
                            {definition.categories.map(category => <Badge key={category} variant="outline" className="bg-blue-50 border-blue-100 text-blue-700 dark:bg-blue-900/30 dark:border-blue-800 dark:text-blue-300 text-xs px-3 py-0.5 rounded-full">
                    ...
                    <div className="space-y-6">
                      <div className="bg-gray-50 dark:bg-gray-800/50 rounded-md p-5 space-y-4">
                        ...
                        <div>
                          ...
                          {definition?.repository ? <a href={definition.repository} className="text-sm text-blue-600 flex items-center hover:underline" target="_blank" rel="noopener noreferrer">
                              <Globe className="h-4 w-4 mr-2" />
                              <span className="truncate">
                                {definition.repository}
                              </span>
                            </a> : <span className="text-sm text-gray-500">Not available</span>}
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <TabsContent value="tools" className="mt-0 pt-0 h-[500px] overflow-auto">
                <div className="p-6">
                  {definition?.tools && definition.tools.length > 0 ? <ServerToolsList tools={definition.tools} isDiscoveryView={false} /> : <div className="flex flex-col items-center justify-center py-16 text-center">
