  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="text-2xl font-semibold">View Configuration</DialogTitle>
            ...
            <div className="w-full p-3 bg-background rounded-lg border">
              <pre 
                ...
                dangerouslySetInnerHTML={{
                  __html: configContent.replace(
                    /("mcpnow"\s*:\s*{(?:[^{}]|{(?:[^{}]|{[^{}]*})*})*})/gs,
                    '<span class="bg-blue-50 border-l-2 border-blue-500 pl-2 block">$1</span>'
                  )
                }}
              />
            </div>
